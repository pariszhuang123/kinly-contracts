...
| `/kinly/market/*` | All marketing campaign pages |
...
| `/kinly/market/<slug>`           | `kinly_market_<slug>`       |
| `/kinly/market/<slug>/<subslug>` | `kinly_market_<slug>_<sub>` |
...
- University poster:  
  `/kinly/market/students?utm_campaign=uni_launch_2026&utm_medium=poster&utm_source=uc_ilam`
...
- Backend resolves `utm_source` at `source_id_resolved`.
...
- Treat first-party conversion CTAs that move the user deeper into Kinly web (e.g., â€œExpress interestâ€, â€œStart on webâ€) as `store = "web"`.
...
- `page_key`: snake_case identifier (e.g., `kinly_general`, `kinly_market_students`)
...
â€¦without compromising user trust.
