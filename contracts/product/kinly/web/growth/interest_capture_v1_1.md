...
- **Surface**: Kinly Web (/get) and withYou Web (/withyou/get)
- **Status**: Active
- **Owners**: Web, DB
...
- campaign-separated lead capture across Kinly and withYou surfaces
...
3. **Idempotent submission per source** Same email + source should not create
   duplicates.
...
- **Inputs**:
  ...
  - `p_source` (text, default `kinly_web_get`) - optional override for other
    campaigns: `kinly_dating_web_get`, `kinly_rent_web_get`, `withyou_web_get`
...
- Key: `(email, source)` (email is case-insensitive via `citext`).
- `deduped = true` when an existing row was updated; `false` when inserted.
- `country_code` and `ui_locale` are overwritten with the latest
  provided values on conflict.
- Same email + different source -> separate row (no conflict).
...
**Deduping key**: `(email, source)` (email is case-insensitive via `citext`)

**Behavior**:

- If email + source already exists: overwrite `country_code`, `ui_locale`; set
  `deduped = true`.
...
- `unique (email, source)` (email via `citext`)
