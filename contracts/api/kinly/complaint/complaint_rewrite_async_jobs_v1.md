...
## 3) Job Lifecycle
States: created → queued → processing → completed; failure path to failed. Transitions are monotonic. (Alias note: if existing rows use `running/succeeded`, map to `processing/completed`.)
...
- `home_id` and `sender_user_id` match persisted rewrite record.  
- `recipient_snapshot_id` references stable recipient set captured at request creation.  
- `source_locale` immutable per request (may be "unknown"); `target_locale` immutable and not "unknown".  
...
- Idempotent per `rewrite_request_id`; retries of enqueue must no-op if job exists.  
- On enqueue failure: mark rewrite failed; return generic failure; no provider calls.

## 6) Processing Rules
### 6.1 Execution
- Lock per request (advisory lock on rewrite_request_id); mark processing.  
- Execute via provider adapter using `routing_decision`; produce output in `target_locale`.  
- Validate against `RewriteResponseV1` and `complaint_rewrite_lexicon_v1` (target locale).  
...
Allowed only if provider supports it AND provider/model/prompt_version/policy_version/execution_mode/lane identical AND source_locale identical (or all unknown) AND target_locale identical.  
...
### 6.3 Locking
- Lock per rewrite_request_id; MUST NOT lock on home_id.
...
- On success: persist RewriteResponseV1 (target_locale), set `rewrite_completed_at`, set `sender_reveal_at = rewrite_completed_at + reveal_delay` (product-defined), mark completed.  
...
- Example D (batchable): all fields match and source_locale="unknown" for some items; allowed if all source_locale are equal OR "unknown".
