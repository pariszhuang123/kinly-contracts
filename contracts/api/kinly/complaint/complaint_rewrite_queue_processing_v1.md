...
- `processing`  : claimed; in progress.
...
```
Notes: raw message text MUST NOT be stored in `routing_decision`. `not_before_at` enables deliberate deferral/batching windows. Timestamps are UTC.
...
If eligible queued jobs exceed `max_jobs_per_run`, process up to the limit and leave the rest queued. Waiting for the next run is normal, not a failure.
...
## 9) Processing modes
Compatible with `execution_mode = async` (per-job provider calls) and `execution_mode = batch` (group by provider/model/prompt_version within the run). Each job STILL needs individual audit trail and final status.
